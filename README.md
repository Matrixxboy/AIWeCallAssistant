...
## Sequence diagram for enhanced speech recognition flow in MessageInput

```mermaid
sequenceDiagram
actor User
participant MessageInput
participant BrowserSpeechRecognition
participant Microphone

User->>MessageInput: Click microphone button
MessageInput->>Microphone: Request audio permission
alt Permission granted
    MessageInput->>BrowserSpeechRecognition: Start recognition
    BrowserSpeechRecognition-->>MessageInput: onstart
    MessageInput->>MessageInput: Set isRecording=true
    BrowserSpeechRecognition-->>MessageInput: onresult (interim/final)
    MessageInput->>MessageInput: Update message
    alt Final transcript
        MessageInput->>MessageInput: handleSend(finalTranscript)
    end
    BrowserSpeechRecognition-->>MessageInput: onend/onnomatch/onerror
    MessageInput->>MessageInput: Set isRecording=false, clear timeout
else Permission denied
    MessageInput->>User: Show error alert
end
Note over MessageInput: Timeout stops recording after 30s
```

#### Class diagram for updated MessageInput component state and methods
...
MessageInput --> SpeechRecognition: uses
```
---
